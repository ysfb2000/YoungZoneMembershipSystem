...
namespace YzMember
{
    public partial class CancelLostForm : Form
    {
        public CancelLostForm()
        {
            InitializeComponent();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("卡号:" + textBox1.Text + "是否恢复正常状态?", "请确认", MessageBoxButtons.OKCancel).ToString() == "OK")
            {
                string strSql = "update yz_Card set Card_State = 1 where Card_NO = '" + textBox1.Text + "'";
                YzMemberClass1.ExecSql(strSql);
                YzMemberClass1.WriteOperateLog("0", "1", "0", "6", textBox1.Text, YzMemberClass1.TAdministrator.Shop.ToString(), YzMemberClass1.TAdministrator.ID.ToString(), textBox2.Text);
                MessageBox.Show("卡号:" + textBox1.Text + ",已恢复成功,可以正常使用!");
